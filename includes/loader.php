...
add_action( 'init', 'legalhelp_bunch_theme_init');

if( !function_exists( 'legalhelp_set' ) ) {
	function legalhelp_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('legalhelp_printr' ) ) {
	function legalhelp_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('legalhelp__font_awesome' ) ) {
	function legalhelp_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = legalhelp_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('legalhelp_load_class' ) ) {
	function legalhelp_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
legalhelp_load_class( 'enqueue', 'helpers', false );

legalhelp_load_class( 'bootstrap_walker', 'helpers', false );

if( legalhelp_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function legalhelp_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include LEGALHELP_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include LEGALHELP_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = LEGALHELP_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = LEGALHELP_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( legalhelp_set( $_GET, 'page' ) == 'legalhelp'.'_option' ) 
		{
			if( legalhelp_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'legalhelp_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = LEGALHELP_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'legalhelp'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'legalhelp'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'legalhelp' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'legalhelp' ), // menu label
	));
